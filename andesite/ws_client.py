...
from . import utils
from .abstract import AbstractAndesiteClient
from .models import LavalinkEvent, LavalinkEventData, LavalinkPlayerState, AndesiteStats
...
class LavalinkWebSocket(AbstractAndesiteClient, EventEmitter):
    _ws: Optional[WebSocketClientProtocol]
    _send_queue: Deque[Dict[str, Any]]

    _stats = Deque[AndesiteStats]
    ...
    @property
    def statistics(self) -> Optional[AndesiteStats]:
        if self._stats:
            return self._stats[0]

    @property
    def all_statistics(self) -> List[AndesiteStats]:
    ...
    async def connect(self):
        ...
        log.debug(f"Connecting to Andesite {self._ws_url} with {headers}...")
        ...
        try:
            ...
            log.info(f"Connected to Andesite {self}!")
    ...
    async def handle_data(self, data: Dict[str, Any]):
        ...
        if op == "event":
            ...
            kwargs = data.copy()
            kwargs.pop("op")
            stats = AndesiteStats.from_data(kwargs)
        ...
        while not self._shutdown_signal:
            try:
                ...
                if await self._attempt_reconnect():
                    ...
                    log.error(f"Unable to reconnect to Andesite ({self})!")
                    break
